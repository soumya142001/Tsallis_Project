...
# -*- coding: utf-8 -*-
"""
Created on Wed May  3 01:01:04 2023
...
beta_list = np.divide(np.ones(len(T)),T)
...
def Tsallis_vdwaal(vars):
    ...
    eq1 = n0(beta,V)+q*n0(beta,V)*xi(beta)*(n0(beta,V)*xi(beta)-1)-((2*v0)/V)*(n0(beta,V))**2-N_avg
...
for beta in beta_list:
    print(beta**(-1))
    V,q =  fsolve(Tsallis_vdwaal,(10*(a**3),0.001))
    if (abs(Tsallis_vdwaal((V,q))[0]) < 10**(-14)) and (abs(Tsallis_vdwaal((V,q))[1]) < 10**(-15)) and V>0:
        print(V/(a**3),q,beta**(-1))
        break 
#sols = nsolve(Tsallis_vdwaal,[beta,q],[1,1])

print(V/(a**3),q)
print(Tsallis_vdwaal((V,q)))
#sols = nsolve(Tsallis_vdwaal,[beta,q],[1,1])
